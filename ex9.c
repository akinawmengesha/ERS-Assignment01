...
int main(void)
{
  ...
  while (1)
  {
    ...
    // Echo mode: Receive character and transmit it back twice
    if (HAL_UART_Receive(&huart2, &uartRxData, 1, 100) == HAL_OK)
    {
      HAL_UART_Transmit(&huart2, &uartRxData, 1, HAL_MAX_DELAY);  // Echo first time
      HAL_UART_Transmit(&huart2, &uartRxData, 1, HAL_MAX_DELAY);  // Echo second time
    }
  }
    /* USER CODE END WHILE */
    MX_USB_HOST_Process();
